                    ...
                    <div class="row">
                        ...
                        <div class="discount-controls">
                            <select name="discount_type" id="discount_type">
                                <option value=""
                                    {{ in_array(old('discount_type', $invoice->discount_type), [null, ''], true) ? 'selected' : '' }}>
                                    None</option>
                                <option value="percentage"
                                    {{ old('discount_type', $invoice->discount_type) === 'percentage' ? 'selected' : '' }}>
                                    %</option>
                                <option value="fixed"
                                    {{ old('discount_type', $invoice->discount_type) === 'fixed' ? 'selected' : '' }}>TZS
                                </option>
                            </select>
                            <input type="number" name="discount_value" id="discount_value"
                                value="{{ old('discount_value', $invoice->discount_value) }}" min="0"
                                step="0.01">
                        </div>
                    </div>
                    <div class="row">
